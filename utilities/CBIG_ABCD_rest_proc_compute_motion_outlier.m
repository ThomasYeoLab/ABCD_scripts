function CBIG_ABCD_rest_proc_compute_motion_outlier(id,output_dir,raw_path,manufacturer,FD_th,DV_th,discard_run,discard_seg,motion_filter)

% CBIG_ABCD_proc_compute_motion_outlier(id,output_dir,raw_path,manufacturer,FD_th,DV_th,discard_run,discard_seg,motion_filter)
%
% Inputs:
%   - id:
%     subject id
...
for i = 1:N_run
    ...
    FD_file = [output_dir '/' id '/bold/mc/' id '_bld' run '_rest_mc_motion_outliers_FDRMS'];
    ...
    motion_par_file = [output_dir '/' id '/bold/' run '/' id '_rest_mc.par'];
    ...
    DVARS_file = [output_dir '/' id '/bold/mc/' id '_bld' run '_rest_mc_motion_outliers_DVARS'];
    if ~exist(DVARS_file,'file')
        DVARS_confound = [output_dir '/' id '/bold/mc/' id '_bld' run '_rest_mc_motion_outliers_confound_DVARS'];
    ...
    if sum(outlier)/length(outlier) >= 1-discard_run/100
        mc_image = [output_dir '/' id '/bold/' run '/' id '_bld' run '_rest_mc.nii.gz'];
