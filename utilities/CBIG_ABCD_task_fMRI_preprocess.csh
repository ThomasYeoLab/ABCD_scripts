#! /bin/csh -f
##########################################
# CBIG ABCD task fMRI Preprocess
##########################################
# AUTHOR #################################
# Jianzhong Chen
# 2020/02/24  
...
##########################################
# This scripts is modified from CBIG_preproc_fMRI_preprocess.csh. The motion correction part is modified so that it will
# skip the motion correction operation since the ABCD minimally processed data has already been corrected for motion
##########################################
# Written by CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md

set VERSION = '$Id: CBIG_ABCD_task_fMRI_preprocess.csh, v 1.0 2020/02/24 $'
...
set BOLD_stem = "_task" # Initialize the BOLD_stem with _task, the BOLD_stem is the input of each step and will be updated after each step
...
set root_dir = "${CBIG_CODE_DIR}/stable_projects/preprocessing/CBIG_fMRI_Preproc2016"
...
foreach step ( "`cat $config`" )
	...
	if ( "$curr_step" == "CBIG_preproc_skip" ) then
		...
		set stem_tmp = $BOLD_stem"_$curr_stem"
		set curr_stem = "skip"
		...
		#rename the file
			...
			if ( -e $output_dir/$subject/bold/$curr_bold/$subject"_bld"$curr_bold$stem_tmp.nii.gz ) then
				mv $output_dir/$subject/bold/$curr_bold/$subject"_bld"$curr_bold$stem_tmp.nii.gz $output_dir/$subject/bold/$curr_bold/$subject"_bld"$curr_bold$BOLD_stem.nii.gz
			endif
		end
	...
	##########################################		
		...
		set mc_stem = "_task_mc"
		set BOLD_stem = "_task_mc"
...
NAME:
	CBIG_ABCD_task_fMRI_preprocess.csh

DESCRIPTION:
	The pipeline processes ABCD minimaly processing task fMRI data and projects the data to 
	...
	(1) [CBIG_preproc_skip -skip 4] 
	    skips first 4 frames of task data. 
...
REQUIRED ARGUMENTS:
	...
	-fmrinii  <fmrinii>        : fmrinii text file including 2 columns, the 1st column contains all run numbers, 
	                            ...
	                            002 /data/../Sub0015_bld002_task.nii.gz
	                            003 /data/../Sub0015_bld003_task.nii.gz
...
OUTPUTS: 
	...
	1. surf folder contains the intermediate and final preprocessed fMRI data on the surface. 
		For example, 
		surf/lh.Sub0033_Ses1_bld002_task_skip4_stc_mc_resid_interp_FDRMS0.2_DVARS50_bp_0.009_0.08_fs6_sm6_fs5.nii.gz 
	...
	2. vol folder contains the intermediate and final preprocessed fMRI data in the MNI152 and freesurfer nonlinear 
	   volumetric spaces.
		For example, 
		a. 
		vol/Sub0033_Ses1_bld002_task_skip4_stc_mc_residc_interp_FDRMS0.2_DVARS50_bp_0.009_0.08_MNI2mm_sm6_finalmask.nii.gz
		...
		b. 
		vol/Sub0033_Ses1_bld002_task_skip4_stc_mc_resid_interp_FDRMS0.2_DVARS50_bp_0.009_0.08_FS1mm_MNI1mm_MNI2mm_sm6_finalmask.nii.gz 
	...
	4. bold folder contains the intermediate files for each step.
		...
		For example, Sub0033_Ses1_bld002_task_skip4_stc_mc.nii.gz is the volume after skip -> slice-timing correction -> 
		motion correction
		...
		high-motion outliers. For example, Sub0033_Ses1_bld002_task_skip4_stc_motion_outliers_DVARS is the text file of 
		DVARS value of each frame of Sub0033_Ses1, run 002;	Sub0033_Ses1_bld002_task_skip4_stc_motion_outliers_FDRMS is the 
